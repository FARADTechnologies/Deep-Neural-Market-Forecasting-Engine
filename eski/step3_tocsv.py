...
# --- ZAMANLAMA AYARI (REAL-TIME) ---
BEKLEME_SURESI = 0.5  # Saniye (Gecikmesiz zamanlayıcı için)
...
def parse_single_listing(hit):
    ...
    clean_data = {
        ...
        "truBroker": hit.get("ownerAgent", {}).get("isTruBroker"),
...
def run_parsing_sales():
    ...
    while True:
        ...
        # --- ZAMAN KONTROLÜ (Non-blocking Timer) ---
        if current_time - last_request_time >= BEKLEME_SURESI:
            ...
            print(f"[{simdi_saat}] 🔄 {page_number}. ci sehife data cekilir...", end=" ")
            ...
            # DİKKAT: time.sleep(0.5) BURADAN SİLİNDİ! 
            # Kod artık burada uyumuyor, yukarıdaki if kontrolü ile bekliyor.
